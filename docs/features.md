...
| ライブ経過分 (minute) | ✅ | `match.minute` + `score.duration` + `lastUpdated` から `currentPeriodStart` を逆算。要点: (1) `score.duration === "EXTRA_TIME"` のときだけ延長扱いにすることで **2nd half ロスタイム (m=92 等) を「延長前半 2 分目」と誤判定しない**、(2) anchor に `lastUpdated` (FD 側のスナップショット時刻) を使うことで FD のキャッシュ遅延を吸収、(3) `periodElapsedMinutes` の `floor+1` と合わせるために periodMinute から 1 引いて elapsed_sec を計算 (off-by-one 補正)。`utils/matchTiming.ts` の `liveMinuteLabel` がここから "67'" / "90+3'" / "FT" 等を生成。ポーリング間隔は 30s、キャッシュ TTL は 25s |
